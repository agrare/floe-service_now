...
RSpec.describe Floe::ServiceNow do
  ...
  describe "integration" do
    ...
    context "with mocked HTTP responses" do
      ...
      before do
        allow(Floe::ServiceNow::Incident).to receive(:build_connection).and_return(connection)
      end
      ...
      it "creates an incident end-to-end" do
        ...
        result = runner.run_async!(
          "servicenow://incident/create_incident",
          {"instance_id" => "dev12345", "short_description" => "Test incident"},
          secrets,
          context
        )

        expect(result["method"]).to eq("incident/create_incident")
        expect(result["success"]).to be true
